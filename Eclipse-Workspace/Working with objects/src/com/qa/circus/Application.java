package com.qa.circus;

public class Application {
public static void main(String[] args) {
	//Elephant dumbo = new Elephant();
	//System.out.println(dumbo.colour); 
//	System.out.println(dumbo.legs);
//	
//	Elephant nellie = new Elephant();
//	nellie.legs = 4;
//	System.out.println(dumbo.legs);
//	System.out.println(nellie.legs);
//
//	Elephant jj = new Elephant();
//   dumbo.haveMudBath("blue");
//	System.out.println(dumbo.colour);

//	Elephant dumbo = new Elephant( ctrl + space
	
	
	Elephant dumbo = new Elephant("african", "grey", 2);
    System.out.println(dumbo.species);
    
    Elephant nellie = new Elephant("indian", "grey", 3);
    System.out.println(nellie.species);
    
   
}}

	






